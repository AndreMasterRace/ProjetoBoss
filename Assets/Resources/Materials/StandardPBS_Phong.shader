...
//            sampler2D _OcclusionHeight2;
            ...
            {
                return UnityEdgeLengthBasedTessCull  (v0.vertex, v1.vertex, v2.vertex, _EdgeLength, _Displacement+_DispOffset);
            }
        
            void disp (inout appdata v)
            {
                v.color = float4(smoothstep(_SlopeLimit-_SlopeBlend,_SlopeLimit+_SlopeBlend,abs(dot(_SlopeDirection.xyz,v.normal))),
                ...
                float d = lerp(tex2Dlod(_DispTex, float4(v.texcoord.xy * _DispTex_ST.xy + _DispTex_ST.zw,0,0)).r,
                            lerp( tex2Dlod(_DispTexHeight, float4(v.texcoord.xy * _DispTex_ST.xy + _DispTex_ST.zw,0,0)).r,
                                    tex2Dlod(_DispTexSlope, float4(v.texcoord.xy * _DispTex_ST.xy + _DispTex_ST.zw,0,0)).r, v.color.r), v.color.g)* _Displacement;
